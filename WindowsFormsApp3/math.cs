...
namespace WindowsFormsApp3
{
    public partial class math : Form
    {
        ...
        private void btthem_Click(object sender, EventArgs e)
        {
            ...
            parameter.Add("@Quantity", nsl.Value);
            parameter.Add("@Price", ngia.Value);
            parameter.Add("@IDNCC", cbbncc.SelectedValue);
            parameter.Add("@Type", cbbloai.SelectedValue);
            try
            {
        ...
        private void button5_Click(object sender, EventArgs e)
        {
            ...
            parameter.Add("@Name", Name);
            dtgMathang.DataSource =
                Database.Query("Select * from MATHANG where TENMATHANG like '%'+@Name+'%'", parameter);
        }
        ...
        private bool Check()
        {
            ...
            if (nsl.Value <= 0)
            {
                errorProvider1.SetError(nsl, "Số lượng phải lớn hơn 0");
                return false;
            }

            if (ngia.Value <= 0)
            {
                errorProvider1.SetError(ngia, "Giá phải lớn hơn 0");
                return false;
            }
        ...
        private void dtgMathang_RowEnter(object sender, DataGridViewCellEventArgs e)
        {
            ...
            nsl.Value = (int)dtgMathang.Rows[e.RowIndex].Cells["SOLUONGTON"].Value;
            ngia.Value = int.Parse(dtgMathang.Rows[e.RowIndex].Cells["GIA"].Value.ToString());
            cbbloai.SelectedValue = dtgMathang.Rows[e.RowIndex].Cells["IDLOAIHANG"].Value;
            cbbncc.SelectedValue = dtgMathang.Rows[e.RowIndex].Cells["IDNHACUNGCAP"].Value;
        }
    }
}